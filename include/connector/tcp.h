...
inline int tcp_send(struct socket * sock, const char * buf, size_t len);
inline int tcp_recv(struct socket * sock, char * buf, size_t max_size);
inline int socket_init(struct socket * sock, struct task_struct * thread);
...
inline int socket_init(struct socket * sock, struct task_struct *thread) {
    ...
    while (retry_count <= max_retries) {
        err = sock -> ops -> connect(sock, (struct sockaddr * ) & saddr, sizeof(saddr), O_RDWR);
        if (err < 0) {
...
inline int tcp_send(struct socket * sock, const char * buf, const size_t len) {
    struct msghdr msg;
    struct kvec vec;
    int length;
